...
EOS
      ...
      if ($auto === FALSE) {
        ...
        throw new \RuntimeException(<<<'EOS'
The login to the host failed.
The host is not configured to accept the type of communications done by this software. This may be due to a configuration problem with the host's web services. Please contact the host's administrator.
EOS
          );
      }
...
EOS
  ...
  public function upload(string $localPath, string $remotePath) {
    ...
    if (file_exists($localPath) === FALSE) {
      throw new \InvalidArgumentException(<<<'EOS'
...
The current connection with the host is not using cookie-based authentication, which is required for file uploading. Please contact the host administrator to enable cookie-based authentication.
EOS
        );
    }
