...
class Utilities {
  ...
  public static function createFolderPath(
    ...
    // Link to root folder groups
    // --------------------------
    // The groups page lists well-known root folder groups (e.g. "personal").
    $r = Constants::ROUTE_ROOTFOLDERS_GROUPS;
    if ($createLinks === TRUE) {
      $groupsLink = Utilities::createRouteLink($r);
    }
    else {
      $groupsLink = t($routeProvider->getRouteByName($r)->getDefault('_title'));
    }

    //
    // Link to root folder group
    // -------------------------
    // The group page lists root folders in the group.
    ...
    $path = $groupsLink . $separator . $groupLink;
    if ($item === NULL) {
      return $path;
    }
    ...
    // Root folder only
    // ----------------
    // When the entity is a root folder, show an abbreviated path that
    // includes links to the groups page and specific group page. If
    // requested, include the root folder at the end.
    if ($item->isRootFolder() === TRUE && $includeFolder === FALSE) {
      return $path;
    }
