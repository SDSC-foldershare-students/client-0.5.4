...
interface FolderShareInterface extends ContentEntityInterface, EntityOwnerInterface, EntityChangedInterface {
  ...
  /**
   * Returns TRUE if access is explicitly granted to a user on this root folder.
   *
   * The user ID is looked up in the root folder's access grants to see if
   * the user has been explicitly granted access. Valid access strings are
   * 'view', 'author', and 'disabled'. A user with 'view' access can view
   * root folder fields and contents.  A user with 'author' access can
   * create, update, and delete fields and contents.  A user with 'disabled'
   * access has no view or author access, but they remain associated with
   * the root folder and may have access in the future, if so granted.
   *
   * Only root folders have access grant user IDs.
   *
   * If a root folder grants access to the anonymous user (uid = 0), then
   * this method returns TRUE for all given user IDs for 'author' and 'view',
   * but not for 'disabled' checks.
   *
   * @param int $uid
   *   The user ID of a user granted access.
   * @param string $access
  ...
  /**
   * Returns a list of descendant IDs, restricted to folders.
   *
  ...
  /**
   * Returns a list of descendant IDs, including files and foldes.
   *
   * List entries are ordered so that the children of a folder are
   * always later in the list than the parent.
   *
   * @return int[]
   *   An ordered list of entity IDs as array values.
   *
   * @see ::getAncestorFolderIds()
   * @see ::getDescendantIds()
   * @see ::isAncestorOfFolderId()
