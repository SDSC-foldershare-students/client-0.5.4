...
class FolderShareUsage {
  ...
  /**
   * Rebuilds the usage table.
   *
   * The usage table is cleared and rebuilt by querying all files and
   * folders and updating the number of root folders, folders, files,
   * and bytes used for all users.
   *
   * This can be time-consuming when the module is managing a large
   * number of files and folders. It is not recommended.
   */
  public static function rebuildAllUsage() {
    // Clear the table.
    self::clearAllUsage();

    // Get all item IDs. This could be a big list.
    $ids = FolderShare::getAllIds();

    // Loop through all IDs and update a tally per user.
    $usageByUser = [];
    foreach ($ids as $id) {
      ...
      $uid = $item->getOwnerId();
      if (isset($usageByUser[$uid]) === FALSE) {
        $usageByUser[$uid] = [
          'nRootFolders' => 0,
          'nFolders'     => 0,
          'nFiles'       => 0,
    ...
    // Update the database.
    foreach ($usageByUser as $uid => $usage) {
