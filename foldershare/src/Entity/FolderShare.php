...
class FolderShare extends ContentEntityBase implements FolderShareInterface {
   ...
   *
   * This method is private. It is used by updateSizes() to recursively
   * change sizes on folders in a folder tree by recursing downward through
   * a folder's children. As all children of this folder are updated, this
   * folder is updated.
   *
  ...
  private function updateSizeDownwardInternal(bool $overwrite = FALSE) {
    ...
    // Validate
    // --------
    // If $this is not a folder or root folder, just return its size.
    $size = $this->getSize();
    if ($this->isFolderOrRootFolder() === FALSE) {
      // This is a file, image, or media item. It has no children, so
      // there is no need for downward recursion.
      return $size;
    }

    // If $overwrite is FALSE and this folder's size is non-empty, then
    // leave the size as-is and return it.
    if ($overwrite === FALSE && $size !== (-1)) {
    ...
    foreach ($this->getChildFolders() as $folder) {
      $size += $folder->updateSizeDownwardInternal($overwrite);
    }
   ...
   *
   * This method is private. It is used by updateSizes() to recursively
   * change sizes on folders in a folder tree by recursing upward through
   * a folder's ancestors. All ancestors of this folder are updated.
   *
   * If this item is a root folder or file, this method has no effect and
  ...
  private function updateSizeUpwardInternal() {
    ...
    // Validate
    // --------
    // If this is not a folder or root folder, we're done.
    if ($this->isFolderOrRootFolder() === FALSE) {
      return;
    }
    ...
    // Update this folder
    // ------------------
    // Set this folder's size *without* recursing. Just sum the sizes of
    // all child files and folders.
    $size = 0;
  ...
  /**
   * Updates folder storage sizes, in bytes.
   *
   * The given array contains a list of entity IDs for folders who's
   * content has presumably changed and now requires folder size updates.
   * Updating proceeds in two steps:
   * - Folder + descendent update.
   * - Ancestor update.
   *
   * During a descendent update, a recursive traversal descends down to
   ...
   * folder's root folder. At each step, a parent folder is updated to be
   * the sum of its immediate children. Afterwards, the parent's parent is
   * processed in the same way, up to and including the root folder. Note
   * that updating a parent *does not* recursively descend through all of
   * that parent's children. We assume that the child sizes are already
   * uptodate and simply sum them to set the parent's size.
   *
   * Complicating things is that this function may take a long time if a
   * folder tree is deep or wide. In that case, the operation may get aborted
   * by hitting an execution limit. If invoked from a worker queue, the
   * task will remain in the queue and be restarted. To avoid a restart
   * starting over from scratch, we need to calculate and save sizes as
   * we go and have some way of knowing how much has been done already.
   *
   * The method used to know what is already uptodate has two parts,
   * corresponding to the two update steps:
   * - During a descendent update, if a folder already has a non-empty size,
   *   that size is retained and recursing down into that folder is skipped.
   * - During an ancestor update, folder sizes are always updated, even
   *   if they are non-empty.
   *
   * The first behavior of skipping descendent recalculations of non-empty
   * sizes may be overwridden by setting the $overwrite parameter to TRUE.
   *
   * The second behavior of always overwriting ancestor folder sizes cannot
   * be overridden.
   *
   * The given array contains entity IDs of the folders to update. If the
   * array is empty, no updates are done.
   *
   * Invalid folder IDs in the array are silently skipped. Such IDs can occur
   * when a queue worker has been tasked with updating folder sizes, but
   * before it runs the folder is deleted by a user.
   *
   * Throughout, when a folder's size is updated, that folder is saved.
   * This means that this method generates *a lot* of database queries to
   * get items, and writes to save results.
   *
   * NOTE: Setting $overwrite to TRUE is discouraged since it can cause a
   * traversal to do a lot of work on big folder trees. This may hit an
   * execution time limit and cause incomplete results. Queue workers should
   ...
   *
   * @param array $itemIds
   *   The entity IDs of folders whose sizes need to be updated.
   * @param bool $overwrite
   *   (optional, default = FALSE) When TRUE, each descendent folder's size is
   *   updated to the summed size of its children, recursively, without
   *   checking if the folder already has a size. When FALSE, a folder's
   *   size is only updated if it is empty.
   *
   ...
   * @see \Drupal\file\FileInterface::getSize()
   */
  public static function updateSizes(array $itemIds, bool $overwrite = FALSE) {
    //
    // Validate
    // --------
    // If the entity ID array is empty, there's nothing to do.
    ...
    // Process
    // -------
    // Loop through the entity IDs and perform two updates:
    //
    // - Descend through children, recursively, and update sizes that are
    //   empty. Do not overwrite children that already have sizes. This
    //   insures that if a queue worker is aborted, any sizes already set
    //   will be re-used when the same task is restarted. This prevents
    //   infinite repeated and aborted attempts to set sizes for a big
    //   folder tree.
    //
    // - Ascend through parents to the root folder, recursively, and update
    //   sizes of parent folders even if they are already set. But do not
    //   recurse back downwards through their children, which would be
  ...
  public static function enqueueUpdateSizes(array $itemIds = []) {
    ...
    if ($queue !== NULL) {
      ...
      self::updateSizes($itemIds, FALSE);
    }
  }
  ...
  public function changeOwnerId(int $newUid) {
    ...
    $size = $this->getSize();
    FolderShareUsage::updateUsageBytes($priorUid, -$size);
    FolderShareUsage::updateUsageBytes($newUid, $size);
  }
  ...
  private function changeAllOwnerIdInternal(int $uid, array &$usageChange) {
    if (isset($usageChange[$uid]) === FALSE) {
      $usageChange[$uid] = [
        'nRootFolders' => 0,
        'nFolders'     => 0,
        'nFiles'       => 0,
    ...
    foreach ($childIds as $id) {
      ...
      if ($child->isFile() === TRUE ||
          $child->isImage() === TRUE ||
          $child->isMedia() === TRUE) {
        // Change the ownership of this item.
        // Insure the new owner has the default access grants.
        ...
        elseif ($child->isMedia() === TRUE) {
          $media = $child->getMedia();
          if ($media !== NULL) {
            $media->setOwnerId($uid);
            $media->save();
          }
        }
    ...
    elseif ($nExceptions > 1) {
      throw new LockException(t(
        Messages::LOCKED_ITEMS,
        [
          '@operation' => 'change ownership',
        ]));
    }
  ...
  public function isAccessGranted(int $uid, string $access) {
    ...
    // Loop through the appropriate view, author, or disabled fields and
    ...
    switch ($access) {
      case 'author':
        ...
        foreach ($this->grantauthoruids->getValue() as $item) {
          ...
          if ($itemUid === $anonymousId || $itemUid === $uid) {
            return TRUE;
          }
        }
        return FALSE;
   ...
   * - "public" = the item is owned by Anonymous or is shared with them, and
   *   it may or may not be owned by the current user.
   *
   * - "personal" = the item is owned by the current user and not shared.
   *
   * - "shared by you" = the item is owned by the current user and is shared
   *   with someone.
   *
   * - "shared with you" = the item is owned by someone other than the
   *   current user, and it is shared with the current user.
   *
   * - "private" = the item is not owned by the current user and it is
   *   not shared, or if sharing is disabled. Access is presumably limited
  ...
  public function getSharingStatus() {
    ...
    // Set up
    // ------
    // Get the root folder, its owner, and information about the current
    // and anonymous users.
    ...
    // Check for disabled
    // ------------------
    // If sharing is disabled site wide, nothing can be shared. The item is
    // either personal (owned by the current user) or private (owned by
    // someone else).
    ...
    $isSharedWithNotAnonymous = FALSE;
    ...
    //
    // - If the item is not shared with anyone but anonymous, but anonymous
    //   sharing has been disabled for the site, then the item is either
    //   personal or private.
    //
    ...
    //
    if ($anonAllowed === FALSE && $isSharedWithNotAnonymous === FALSE) {
      return ($rootOwnerId === $currentUserId) ? 'personal' : 'private';
    }
  ...
  public function getDescendantIds() {
    ...
    foreach ($childIds as $id) {
      ...
      // Recurse to add its children, if any.
      ...
      try {
        ...
        try {
          ...
          throw new ValidationException(t(
            Messages::CORRUPTED_DATABASE,
            [
              '@method' => 'FolderShare::setAllRootFolderIdInternal',
            ]));
        }
  ...
  public static function parseAndValidatePath(string $path) {
    ...
    if ($slashslash === '//') {
      ...
      if ($endOfUser === FALSE) {
        ...
        throw new ValidationException(t(
          Messages::PATH_MISSING_FOLDER_NAME,
          []));
      }
  ...
  public static function createRootFolder(
    ...
    if (self::acquireRootLock() === FALSE) {
      throw new LockException(t(
        Messages::LOCKED_ROOT_FOLDERS,
        [
          '@operation' => 'folder creation',
        ]));
    }
  ...
  public function copyToRoot(bool $adjustName = TRUE, string $newName = '') {
    ...
    if ($this->isFolderOrRootFolder() === FALSE) {
      throw new ValidationException(t(
        Messages::API_INVALID_COPY_TO_ROOT,
        [
          '@method' => 'FolderShare::copyToRoot',
        ]));
    }
  ...
  public function copyToFolder(
    ...
    // Verify that the copy destination is not this folder.
    if ((int) $parent->id() === (int) $this->id()) {
      throw new ValidationException(t(
        Messages::API_INVALID_CIRCULAR,
        [
          '@name' => $this->getName(),
          '@method' => 'FolderShare::copyToFolder',
          '@operation' => 'copy',
        ]));
    }

    // Verify that the copy destination is not deeper down in this item.
    //
    ...
    // Execute
    // -------
    // Copy this item. The following variants are handled:
    //
    // - If $newName is not empty, then the copied item will be renamed
    //   to have the new name. Name collisions are handled based on that
    //   new name.
    ...
    try {
      $copy = $this->copyToFolderInternal(
        $parent,
        $adjustName,
        $newName,
        $usageChange);
    ...
    // The copy operation may or may not have completed with a full copy.
    // If some items could not be copied, then an exception will have been
    // thrown and only some items copied. In the latter case, subfolders
    // that were completely copied will have updated sizes. But partially
    // copied items will not have empty sizes (since setting the size is
    // the last step of a folder copy).
    //
    // If a copy object has been returned, then the copy completed and
    // there should be a size on the folder. We still need to update the
    // sizes for all folder ancestors, but that is quick. We can do that
    // immediately without queueing a background task.
    //
    // If a copy object has not been returned, then there still may be
    // new content in the destination, but something didn't copy properly.
    // In this case, the destination may now have new content in it, but
    // some of it may not be complete. Updating sizes requires a deep
    // traversal, and this could take time. Queue it as a background task.
    if ($copy === NULL) {
      // An error occurred. We're going to have to queue the size update
      // because it might require a deep traversal. The update needs to
      // start on the parent.
      $updateId = (int) $parent->id();
      $queueUpdate = TRUE;
    }
    else {
      // An error did not occur. The copied item and all children should
      // all have the same sizes the original had, so they don't need to
      // be updated. But the parent and its ancestors need updates. This
      // is quick, so do it now.
      $updateId = (int) $parent->id();
      $queueUpdate = FALSE;

      self::updateSizes([$updateId], FALSE);
    }
    ...
    // sizes, if needed.
    if ($queueUpdate === TRUE) {
      self::enqueueUpdateSizes([$updateId]);
    }
  ...
  private function copyToFolderInternal(
    ...
    if ($this->isFolderOrRootFolder() === FALSE && $dstParent === NULL) {
      throw new ValidationException(t(
        ...
        [
          '@name' => $this->getName(),
          '@method' => 'FolderShare::copyToFolderInternal',
          '@operation' => 'copy',
        ]));
    }
    ...
    elseif (isset($avoidNames[$copyName]) === TRUE) {
      ...
      if ($dstParent === NULL) {
        ...
        throw new ValidationException(t(
          Messages::NAME_IN_DESTINATION_IN_USE,
          [
            '@name'        => $copyName,
            '@destination' => $dstParent->getName(),
            '@operation'   => 'copy',
          ]));
      }
    }
  ...
  public function delete() {
    ...
    // Update sizes. If the item has a parent folder, that parent's size
    // is now out of date, and so are its ancestors.
    if ($parentId !== (-1)) {
      self::updateSizes([(int) $parentId], FALSE);
    }
   ...
   *   has keys for user IDs, and values that are arrays reporting the usage
   *   change.
   *
   * @throws \Drupal\foldershare\Entity\Exception\LockException
   *   If an access lock could not be acquired.
  ...
  public function moveToRoot(string $newName = '') {
    ...
    if ($this->isFolderOrRootFolder() === FALSE) {
      throw new ValidationException(t(
        Messages::API_INVALID_MOVE_TO_ROOT,
        [
    ...
    $oldParentId = (int) $this->getParentFolderId();
    ...
    if ($k === self::FOLDER_KIND) {
      ...
      // This increases the number of root folders, and decreases
      // the number of subfolders.
      $ownerId = $this->getOwnerId();
      FolderShareUsage::updateUsageRootFolders($ownerId, 1);
      FolderShareUsage::updateUsageFolders($ownerId, -1);
    }
    ...
    // Update folder size for the old parent of the folder. Since the folder
    // is now a root folder, there is no new parent to update.
    self::updateSizes([$oldParentId], FALSE);
  ...
  public function moveToFolder(
    ...
    if ($dstParent->isFolderOrRootFolder() === FALSE) {
      throw new ValidationException(t(
        Messages::API_ITEM_NOT_FOLDER,
        [
          '@method' => 'FolderShare::moveToFolder',
        ]));
    }

    // If this folder's parent is the destination, then this is really
    // a rename and we can skip a few steps below. But if no new name
    // was provided, then it is a move from and to the same folder,
    // which is pointless.
    $dstParentId = (int) $dstParent->id();
    $parentId = (int) $this->getParentFolderId();
    $justRename = FALSE;
    if ($parentId === $dstParentId) {
      if (empty($newName) === TRUE) {
        // Move to same location with same name. Do nothing.
        return;
      }

      // Move to same location with a different name. Just rename.
      $justRename = TRUE;
    }
    ...
    // LOCK THIS FOLDER TREE.
    if ($justRename === TRUE) {
      // Just lock the item, not the entire folder tree.
      if ($this->acquireLock() === FALSE) {
        throw new LockException(t(
          Messages::LOCKED_ITEM,
          [
    ...
    elseif (isset($dstParent->getChildNames()[$nameToCheck]) === TRUE) {
      $savedException = new ValidationException(t(
        Messages::NAME_IN_DESTINATION_IN_USE,
        [
          '@name'        => $nameToCheck,
          '@destination' => $dstParent->getName(),
    ...
    //
    // Execute simple rename case
    // --------------------------
    // If the item isn't moving, just renaming, then there is less to do.
    // Just update the name, but leave the parent, root, kind, and size
    ...
    // Execute full move
    // -----------------
    // When the item is moving, change the parent ID, change the kind (if
    // needed), update access controls (if needed), update the name (if needed),
    // and change the root ID (if needed).
    if ($justRename === FALSE) {
      ...
      if ($k === self::ROOT_FOLDER_KIND) {
        ...
        // This increases the number of subfolders, and decreases
    ...
    // Update folder sizes for this parent and the destination parent.
    if ($justRename === FALSE) {
      self::updateSizes(
        [
          $dstParentId,
  ...
  private function addFilesInternal(
    ...
    if ($checkForInFolder === TRUE) {
      foreach ($files as $index => $file) {
        ...
        if ($parentFolderId !== -1) {
          ...
          // Complain.
          $v = new ValidationException(t(
            Messages::API_ITEM_ALREADY_IN_FOLDER,
            [
              '@method' => 'FoldeRShare::addFilesInternal',
            ]));
    ...
    // affect this folder (and all ancestor folders) and their sizes are
    // already available in their File objects, which are already in
    // cache. Updating this folder's size should be quick, so do it now.
    self::updateSizes([(int) $this->id()], FALSE);
  ...
  public function archiveToZip(array $children) {
    ...
    if ($this->isFolderOrRootFolder() === FALSE) {
      throw new ValidationException(t(
        Messages::API_ITEM_NOT_FOLDER,
        [
          '@method' => 'FolderShare::archiveToZip',
        ]));
    }

    if (empty($children) === TRUE) {
      throw new ValidationException(t(
        Messages::API_ARRAY_EMPTY,
        [
          '@method' => 'FolderShare::archiveToZip',
        ]));
    }
    ...
    foreach ($children as $child) {
      ...
      if (in_array($child->id(), $childIds) === FALSE) {
        ...
        throw new ValidationException(t(
          Messages::API_ITEM_NOT_IN_FOLDER,
          [
            '@method' => 'FolderShare::archiveToZip',
          ]));
      }
    ...
    if (empty($extensionsString) === FALSE) {
      ...
      if ($zipFound === FALSE) {
        throw new ValidationException(t(
          Messages::API_ARCHIVE_UNSUPPORTED,
          [
            '@method' => 'FolderShare::archiveToZip',
          ]));
      }
    ...
    if (empty($archiveUri) === TRUE) {
      ...
      if ($archiveUri === FALSE) {
        ...
        throw new SystemException(t(
          Messages::OS_CANNOT_CREATE_FiLE,
          [
            '@path' => $archiveUri,
          ]));
      }
    ...
    switch ($this->getKind()) {
      ...
      case self::ROOT_FOLDER_KIND:
        ...
        if ($archive->addEmptyDir($currentZipPath) === FALSE) {
          throw new SystemException(t(
            Messages::OS_CANNOT_ADD_TO_ARCHIVE,
            [
    ...
    if (empty($extensionsString) === FALSE) {
      ...
      for ($i = 0; $i < $numFiles; $i++) {
        ...
        if (FileUtilities::isNameExtensionAllowed($path, $extensions) === FALSE) {
          $archive->close();
          throw new systemException(t(
            Messages::FILETYPE_IN_ARCHIVE_NOT_ALLOWED,
            [
              '@name' => $path,
            ]));
        }
  ...
  private static function inputDataToFile() {
    ...
    if ($stream === FALSE) {
      throw new SystemException(t(
        Messages::OS_CANNOT_READ_INPUT_STREAM,
        []));
    }
    ...
    if ($temp === FALSE) {
      fclose($stream);
      throw new SystemException(t(
        Messages::OS_CANNOT_CREATE_TEMP_FILE,
        [
          '@path' => $tempUri,
        ]));
    }
