...
class FolderShareBreadcrumbBuilder implements BreadcrumbBuilderInterface {
  ...
  public function applies(RouteMatchInterface $routeMatch) {
    ...
    switch ($routeName) {
      ...
      case Constants::ROUTE_ROOTFOLDERS_GROUPS:
        // The route is for a page listing well-known root folder groups.
        return TRUE;
      ...
      default:
        // Check the list of well-known root folder groups.
        foreach (Constants::ROOTFOLDERS_GROUPS as $group) {
          if ($routeName === $group['route']) {
            // The route is for a root folder group.
            return TRUE;
          }
        }
  ...
  public function build(RouteMatchInterface $routeMatch) {
    ...
    switch ($routeName) {
      ...
      case Constants::ROUTE_ROOTFOLDERS_GROUPS:
        // The route is for the list of root folder groups.
        return $this->buildForRootFolderGroupsPage($routeMatch);
      ...
      default:
        ...
        foreach (Constants::ROOTFOLDERS_GROUPS as $group) {
          if ($routeName === $group['route']) {
            // The route is for a root folder group.
            return $this->buildForRootFolderGroupPage($routeMatch);
          }
        }
  ...
  private function buildDefault(RouteMatchInterface $routeMatch) {
    ...
    // Link to groups page
    // -------------------
    // The second link goes to the list of root folder groups.
  ...
  /**
   * Builds an array of breadcrumb links for the group list page.
   *
   * When a page lists well-known root folder groups (e.g. "personal"),
   * there are no ancestor entities. Instead, this function returns an
   * abbreviated breadcrumb that only includes a home page link.
   *
   * @param \Drupal\Core\Routing\RouteMatchInterface $routeMatch
   *   The route description, including the name of the route and its
   *   parameters.
   ...
   *   page to the current FolderShare entity.
   */
  private function buildForRootFolderGroupsPage(RouteMatchInterface $routeMatch) {
    //
    // Cache control
    // -------------
    // Breadcrumbs vary per user because viewing permissions
    ...
    // Link to home page
    // -----------------
    // The first (and only) link goes to the site's home page.
    $links   = [];
    $links[] = Link::createFromRoute($this->t('Home'), '<front>');
   ...
   *   page to the current FolderShare entity.
   */
  private function buildForRootFolderGroupPage(
  ...
  private function buildForEntityPage(
    ...
    // Link to group page
    // ------------------
    // The third link goes to the root folder group.
    //
    // Decide on the the appropriate group by looking at the root folder
    // of the current item.
    $rootFolder = $item->getRootFolder();
    switch ($rootFolder->getSharingStatus()) {
      default:
      case 'private':
        // Root folder is owned by someone else and it has not been shared with
