 ...
 *  pageTitle       = @Translation("Share @operand Folder Tree"),
 *  menuNameDefault = @Translation("Share Folder Tree..."),
 *  menuName        = @Translation("Share Folder Tree of @operand..."),
...
class ShareItem extends FolderShareCommandBase {
  ...
  public function buildConfigurationForm(
    ...
    if ($item->isRootFolder() === TRUE) {
      $description = t(
        Messages::SHARE_DESCRIPTION_THIS,
        []);
    }
    else {
      $description = t(
        Messages::SHARE_DESCRIPTION_THIS_ROOT,
        [
          '@kind' => Utilities::mapKindToTerm($item->getKind()),
          '@name' => $item->getName(),
        ]);
  ...
  private function buildRow(
    ...
    if ($isAnonymous === TRUE) {
      ...
      if ($couldView === FALSE) {
        $message = t(
          Messages::SHARE_ANONYMOUS_NO_VIEW,
          [
            '@account' => mb_convert_case($name, MB_CASE_TITLE),
          ]);
        $rowDisabled = TRUE;
      }
      elseif ($couldAuthor === FALSE) {
        $message = t(
          Messages::SHARE_ANONYMOUS_NO_AUTHOR,
          [
            '@account' => mb_convert_case($name, MB_CASE_TITLE),
          ]);
      }
      ...
      if ($isBlocked === TRUE) {
        $message = t(Messages::SHARE_USER_BLOCKED);
        $rowDisabled = TRUE;
      }
      elseif ($isAdmin === TRUE) {
        $message = t(Messages::SHARE_USER_IS_ADMIN);
      }
      elseif ($isOwner === TRUE) {
        $message = t(Messages::SHARE_USER_IS_OWNER);
      }
      elseif ($couldView === FALSE) {
        $message = t(Messages::SHARE_USER_NO_VIEW);
        $rowDisabled = TRUE;
      }
      elseif ($couldAuthor === FALSE) {
        $message = t(Messages::SHARE_USER_NO_AUTHOR);
      }
    }
