 ...
 *  pageTitle       = @Translation("Copy @operand"),
 *  menuNameDefault = @Translation("Copy..."),
 *  menuName        = @Translation("Copy @operand..."),
...
class CopyItem extends FolderShareCommandBase {
      ...
      foreach ($selection as $items) {
        foreach ($items as $item) {
          if ($destination->isDescendantOfFolderId($item->id()) === TRUE) {
            throw new ValidationException(t(
              Messages::COPY_TO_SUBFOLDER_SELF,
              []));
          }
        }
      }
  ...
  public function execute() {
    ...
    elseif (count($selection) === 1) {
      ...
      $this->addExecuteMessage(t(
        Messages::COPY_DONE_MULTIPLE_ITEMS,
        [
          '@number' => $nItems,
    ...
    if ($nKinds === 1) {
      if ($nItems === 1) {
        ...
        if ($firstKind === FolderShare::FOLDER_KIND ||
          ...
          $description = t(
            Messages::COPY_DESCRIPTION_ONE_FOLDER,
            [
              '@kind' => $operandName,
            ]);
        }
          ...
          $description = t(
            Messages::COPY_DESCRIPTION_ONE_FILE,
            [
              '@kind' => $operandName,
            ]);
        }
        ...
        if ($firstKind === FolderShare::FOLDER_KIND ||
            $firstKind === FolderShare::ROOT_FOLDER_KIND) {
          $description = t(
            Messages::COPY_DESCRIPTION_MULTIPLE_ITEMS,
            [
              '@kinds' => $operandName,
            ]);
        }
          ...
          $description = t(
            Messages::COPY_DESCRIPTION_MULTIPLE_FILES,
            [
              '@kinds' => $operandName,
            ]);
        }
