...
abstract class FolderShareCommandBase extends PluginBase implements ConfigurablePluginInterface, FolderShareCommandInterface {
  ...
  protected function getDestinationId() {
    $id = $this->configuration['destinationId'];
    if (empty($id) === TRUE) {
      $id = (-1);
    }
    return $id;
  ...
  protected function getDestination() {
    ...
    if (empty($destinationId) === TRUE || $destinationId === (-1)) {
      return NULL;
    }
  ...
  public function validateParentConstraints() {
    ...
    if ($this->getParentId() === (-1)) {
      ...
      if ($parent === NULL) {
        throw new ValidationException(t(
          Messages::API_COMMAND_INVALID_PARENT_ID,
          [
            '@command' => $label,
            '@id'      => $this->getParentId(),
          ]));
    ...
    if ($parentKind === 'none' && in_array('none', $kinds) === FALSE) {
      ...
      // no parent.
      throw new ValidationException(t(
        Messages::API_COMMAND_UNSUPPORTED_CONTEXT,
        [
          '@command' => $label,
        ]));
    }
  ...
  public function validateSelectionConstraints() {
    ...
    if ($nSelected === 0) {
      ...
      if ($parentId !== (-1) && in_array('parent', $types) === TRUE) {
        ...
        if ($canHaveMany === TRUE) {
          throw new ValidationException(t(
            Messages::API_COMMAND_NEED_MULTIPLE_SELECTION,
            [
              '@command' => $label,
            ]));
        }
        ...
        throw new ValidationException(t(
          Messages::API_COMMAND_NEED_SINGLE_SELECTION,
          [
            '@command' => $label,
          ]));
      }
    ...
    if (in_array('any', $kinds) === FALSE) {
      ...
      foreach (array_keys($selection) as $kind) {
        if (in_array($kind, $kinds) === FALSE) {
          throw new ValidationException(t(
            Messages::API_COMMAND_UNSUPPORTED_KIND,
            [
              '@command' => $label,
              '@kinds'   => Utilities::mapKindToTerms($kind),
            ]));
        }
      }
    }
      ...
      foreach ($selection as $kind => $items) {
        foreach ($items as $item) {
          if ($item->getParentFolderId() !== $parentId) {
            throw new ValidationException(t(
              Messages::API_COMMAND_NOT_ALL_CHILDREN,
              [
                '@command' => $label,
                '@id'      => $item->id(),
              ]));
  ...
  public function validateDestinationConstraints() {
    ...
    if (empty($this->getDestinationId()) === TRUE ||
        $this->getDestinationId() === (-1)) {
      ...
      if ($destination === NULL) {
        ...
        throw new ValidationException(t(
          Messages::API_COMMAND_INVALID_DESTINATION_ID,
          [
            '@command' => $label,
