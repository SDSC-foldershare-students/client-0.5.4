 ...
 *  pageTitle       = @Translation("Delete @operand"),
 *  menuNameDefault = @Translation("Delete..."),
 *  menuName        = @Translation("Delete @operand..."),
...
class DeleteItem extends FolderShareCommandBase {
  ...
  public function execute() {
    ...
    elseif (count($selection) === 1) {
      ...
      $this->addExecuteMessage(t(
        Messages::DELETE_DONE_MULTIPLE_ITEMS,
        [
          '@number' => $nItems,
    ...
    if ($nKinds === 1) {
      if ($nItems === 1) {
        ...
        if ($firstKind === FolderShare::FOLDER_KIND ||
          ...
          $description = t(
            Messages::DELETE_PROMPT_ONE_FOLDER,
            [
              '@kind' => $operandName,
            ]);
        }
          ...
          $description = t(
            Messages::DELETE_PROMPT_ONE_FILE,
            [
              '@kind' => $operandName,
            ]);
        }
        ...
        if ($firstKind === FolderShare::FOLDER_KIND ||
            $firstKind === FolderShare::ROOT_FOLDER_KIND) {
          $description = t(
            Messages::DELETE_PROMPT_MULTIPLE_ITEMS,
            [
              '@kinds' => $operandName,
            ]);
        }
          ...
          $description = t(
            Messages::DELETE_PROMPT_MULTIPLE_FILES,
            [
              '@kinds' => $operandName,
            ]);
        }
  ...
  public function buildConfigurationForm(
    ...
    $form['#title'] = Utilities::createTitle($def['pageTitle'] . '?', $selection);
    $form['description'] = [
      '#type'  => 'html_tag',
      '#tag'   => 'p',
