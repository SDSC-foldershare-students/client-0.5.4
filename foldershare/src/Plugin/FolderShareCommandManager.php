...
class FolderShareCommandManager extends DefaultPluginManager implements CategorizingPluginManagerInterface {
  ...
  private function validateLabel(array &$pluginDefinition) {
    ...
    if (empty($pluginDefinition['pageTitle']) === TRUE) {
      $pluginDefinition['pageTitle'] = $pluginDefinition['label'];
    }
  ...
  private function validateKinds(
    ...
    foreach ($constraints['kinds'] as $index => $k) {
      $lowerk = mb_convert_case($k, MB_CASE_LOWER);
      ...
      switch ($lowerk) {
        ...
        default:
          \Drupal::logger(Constants::MODULE)->error(
            'Invalid FolderShareCommand plugin, ID "' .
            $pluginDefinition['id'] . '": the "kinds" value has an unrecognized "' . $k . '" value.');
          return FALSE;
      }

      $constraints['kinds'][$index] = $lowerk;
    }
   ...
   * plus 'any' and 'rootfoldergroup'.  Kinds are automatically converted to
   * lower case.
   *
   ...
   *
   * 'kinds' values are not validated but should be known FolderShare kinds,
   * plus 'none', 'any', and 'rootfoldergroup'.  Kinds are automatically
   ...
   * - 'rootfoldergroup': the root folder group context.
   *
