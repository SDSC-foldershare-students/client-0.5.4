...
class FolderShareSearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
   ...
   * Configuration form.
   *
   * These functions build a search plugin configuration form, validate
   * it, and record the site administrator's choices from the form.
   *
   *------------------------------------------------------------------*/
  ...
  public function searchFormAlter(array &$form, FormStateInterface $formState) {
    ...
    if (empty($c['search_file_content']) === TRUE ||
      ...
      $description = 'Search for text in folder and file names, filename extensions, descriptions, comments, and owner names.';
    }
    else {
      $description = 'Search for text in folder and file names, filename extensions, descriptions, comments, owner names, and file content.';
    }
  ...
  private function indexItem(FolderShareInterface $item) {
    ...
    $adminWords = [
      Utilities::mapKindToTerm($item->getKind()),
   ...
   * Search.
   *
   * These functions execute a search, retrieving a structured list of
   * search results.
   *
   *------------------------------------------------------------------*/
  ...
  public function execute() {
    ...
    // is (-1), then the search is not constrained to a parent's folder tree.
    $keywords = $this->getKeywords();
   ...
   *   a search phrase.
   * @param int $parentId
   *   (optional, default = (-1) = none) Constrain the search to the
   *   descendants of the folder with the indicated FolderShare entity ID.
   *
   * @return \Drupal\Core\Database\StatementInterface|null
   *   Returns results from a search query, or NULL if the search failed.
   */
  private function search(string $keywords, int $parentId = (-1)) {
    //
    // Query
    // -----
    ...
    if ($hasAdmin === FALSE) {
      ...
      if (Settings::getSharingAllowed() === FALSE) {
        // The site does not allow sharing. The user only has access to
        // items they own. Add a query condition.
        $query->condition('fs.uid', $this->account->id(), '=');
      }
      elseif ($this->account->isAnonymous() === TRUE &&
          Settings::getSharingAllowedWithAnonymous() === FALSE) {
        // The site does allow sharing, but not with anonymous. And the user
        // is anonymous. The anonymous user only has access to items they
        // own. Add a query condition.
        $query->condition('fs.uid', $this->account->id(), '=');
      }
      else {
        // The site does allow sharing, and sharing with anonymous.
