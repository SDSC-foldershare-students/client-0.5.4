...
class UpdateFolderSizes extends QueueWorkerBase {
  ...
  public function processItem($data) {
    ...
    // tree and updating child folders first, then ancestors.
    //
    // If this job hits a timeout, then it will be automatically
    // requeued and tried again.  Any child folders that have already
    // had their sizes updated will have those sizes reused so that
    // size calculations don't have to be repeated. Eventually, then,
    // even a long folder ID list and deep folder tree will get fully
    // processed so that this queue entry can be finished.
    FolderShare::updateSizes($ids, FALSE);
  }
